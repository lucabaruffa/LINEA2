...
public class PLC implements Runnable {
	...
	public PLC(String name,String ip,byte[] plcToPc,byte[] pcToPlc,int plcToPcDb,int pcToPlcDb,double[] booleans,JTextArea monit, String rac , String slo) {
		this.plcToPc = plcToPc;
		this.pcToPlc = pcToPlc;
		this.PLCName = name;
		this.PLCIp = ip;
		...
		this.rack = Integer.parseInt(rac);
		this.slot = Integer.parseInt(slo);
	...
	public void checkSetLiveBit() {
		try {
			...
			if ((System.nanoTime() - this.plcToPcLiveBit) > this.liveBitPLCDuration * 1000000) {
				...
				System.out.println("No live bit from PLC: " + this.PLCName);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void run() {
		while(true) {
			if (this.moka.Connected == false) {
				...
				if (error > 0) {
					System.out.println(S7Client.ErrorText(error));
					
					if (monitor != null)
						monitor.append("570 - "+ S7Client.ErrorText(error)+"\n");
				}
			} else {
				this.connected = true;
				...
				if (this.firstConnect == true) {
					System.out.println("Connected to PLC " + this.PLCName);
					
					if (monitor != null)
						monitor.append("580 -"+ "Connected to PLC " + this.PLCName);
	...
	public void connetti() {
		if (this.moka.Connected == false) {
			...
			if (error > 0) {
				...
				if (monitor != null)
					monitor.append("620 - "+ S7Client.ErrorText(error)+"\n");
			}else {
				if (monitor != null)
					monitor.append("Connesso con SUCCESSO !!\n");
			}
		} 
	}//fine connetti
