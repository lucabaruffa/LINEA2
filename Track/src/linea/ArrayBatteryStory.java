...
public class ArrayBatteryStory {
	...
	private void aggiungi(BatteryStory battstory) {
		...
		synchronized (arrayBatteryStory) {
			...
			if (arrayBatteryStory.size() > MAX_SIZE) {
				log.write("SUPERATE MASSIME DIMENSIONI PER ARRAYBATTERYSTORY. CANCELLO LA PRIMA CODICE:"
						+ arrayBatteryStory.poll().getCodiceBatteryStory());
	...
	public void aggiungiBatteria(Batteria batteria) {
		...
		if (bStory != null) {
			...
			// monitor.append("\nArrayBatteryStory -> La batteria
			// codice:"+batteria.getCodiceBatteria()+" è presente. La AGGIUNGO");
			bStory.aggiungi(batteria);
		} else {
			// creo BatteryStory e la inserisco
			// monitor.append("\nArrayBatteryStory -> La batteria
			// codice:"+batteria.getCodiceBatteria()+" NON è presente. CREO e AGGIUNGO");
