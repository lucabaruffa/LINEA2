...
public class ConfiguratoreLinea {
	...
	public void scanReader() {
		...
		try {
			for (int i = 0; i < NUMERO_STAZIONI_ATTIVE; i++) {
				...
				AtomoConfigurazioneLinea atomo = new AtomoConfigurazioneLinea(statoscanner, scartoabilitato, spare1,
						spare2);

				// log.write("CONFIGURATOR LINEA FOR : PRIMA DEL CONTROLLO. i=" + i + " -
				// postazione di controllo 2=" + (Setting.DB_POSTAZIONE_CONTROLLO2 -1));
				if (i == (NUMERO_STAZIONI_ATTIVE - 1))
					dateList[(Setting.STAZIONE_DI_CONTROLLO_2) - 1] = atomo;
				else
					dateList[i] = atomo;

				log.write("CONFIGURATOR LINEA FOR : statoscanner: " + i + " -> " + statoscanner
						+ "  - scarto abilitato:" + scartoabilitato + " - spare1:" + spare1 + "  - spare2:" + spare2);
	...
	public void DBWrite() {
		...
		if (Result == 0) {
			log.write("ConfiguratoreLinea -> DB " + DB_CONF
					+ " succesfully written using size reported by DBwrite() - Modulo configuratoreLinea");
		} else
	...
	public void connetti() throws Exception {
		...
		int Result = Client.ConnectTo(setting.getIPPLC(), Integer.valueOf(setting.getRACK()),
				Integer.valueOf(setting.getSLOT()));
		if (Result == 0) {
			...
			// System.out.println("Connected to : " + setting.getIPPLC() + " (Rack=" +
			// Integer.valueOf(RACKtext.getText()) + ", Slot=" +
			// Integer.valueOf(SLOTtext.getText())+ ")");
			// System.out.println("PDU negotiated : " +
			// Integer.valueOf(SLOTtext.getText()));
			// statoplc.setText("CONNESSIONE OK!");
		} else {
	...
	public void setListaAtomoConfigurazione(AtomoConfigurazioneLinea[] lista) {
		...
		try {
			...
			for (int i = 0; i < NUMERO_STAZIONI_ATTIVE; i++) {
				...
				log.write("HO SCRITTO SUL PLC: statoscanner:" + atomo.statoscanner + " - scartoabilitato:"
						+ atomo.scartoabilitato + " - OFFSET =" + (i + 1) + " - Modulo ConfiguratoreLinea");
			...
			log.write("\ncONFIGURAZIONE lINEA -> Errore DBAREA:" + e.toString() + "  --  inizio:" + indirizzo_start
					+ "   - Modulo: ConfiguratoreLinea");
		}
